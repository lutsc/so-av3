...
#define SBI_EID 0 // Extension ID

static inline void sbi_set_timer(uint64_t time){
  register uint64_t a0 asm("a0") = time;
  register uint64_t a7 asm("a7") = SBI_EID; // EID do set_timer do SBI
  asm volatile(
    "ecall"
...
void timer_next(void){
  ...
  // lê o valor de tempo do CSR (Control and Status Register) 
...
// // método sem SBI, talvez seja necessário botar extensão SSTC no makefile
// void timer_next(void) {
//   uint64_t now;
...
//   // escreve no CSR stimecmp (0x14D)
//   asm volatile("csrw 0x14D, %0" :: "r"(now + tick_interval));
// }
...
void timer_init(uint64_t interval){
  ...
  // ativa o 5° bit do reg. sie, habilitando interrupção do timer 
